...
```bash
# Test SSH connection
ssh <redacted-pod-id>@ssh.runpod.io -i ~/.ssh/id_ed25519 'echo "Connected to RunPod"'
```

### 2. Setup Repository on RunPod
```bash
# SSH into RunPod
ssh <redacted-pod-id>@ssh.runpod.io -i ~/.ssh/id_ed25519
...
# From local Mac:
# Copy all scripts
scp -i ~/.ssh/id_ed25519 -r /Users/user/MaximalCAI/scripts/* \
    <redacted-pod-id>@ssh.runpod.io:/workspace/cai-constitution-bootstrap/scripts/

# Copy constitution
scp -i ~/.ssh/id_ed25519 /Users/user/MaximalCAI/constitution.yaml \
    <redacted-pod-id>@ssh.runpod.io:/workspace/cai-constitution-bootstrap/

# Copy specs (for reference)
scp -i ~/.ssh/id_ed25519 -r /Users/user/MaximalCAI/specs/* \
    <redacted-pod-id>@ssh.runpod.io:/workspace/cai-constitution-bootstrap/specs/
```
...
scp -i ~/.ssh/id_ed25519 -r \
    <redacted-pod-id>@ssh.runpod.io:/workspace/cai-constitution-bootstrap/results/* \
    /Users/user/MaximalCAI/results/

# Checkpoints (LoRA adapters)
scp -i ~/.ssh/id_ed25519 -r \
    <redacted-pod-id>@ssh.runpod.io:/workspace/cai-constitution-bootstrap/checkpoints/* \
    /Users/user/MaximalCAI/checkpoints/

# Generated data
scp -i ~/.ssh/id_ed25519 -r \
    <redacted-pod-id>@ssh.runpod.io:/workspace/cai-constitution-bootstrap/data/* \
    /Users/user/MaximalCAI/data/

# Logs
scp -i ~/.ssh/id_ed25519 -r \
    <redacted-pod-id>@ssh.runpod.io:/workspace/cai-constitution-bootstrap/logs/* \
    /Users/user/MaximalCAI/logs/
```
...
```bash
python scripts/runpod_manager.py stop <redacted-pod-id>
```
