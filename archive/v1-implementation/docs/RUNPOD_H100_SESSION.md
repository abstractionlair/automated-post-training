...
```bash
ssh root@<redacted-pod-ip> -p 22069 -i ~/.ssh/id_ed25519
```
...
```bash
export RUNPOD_HOST=<redacted-pod-ip>
...
```bash
# Single file
scp -P 22069 -i ~/.ssh/id_ed25519 local_file.txt root@<redacted-pod-ip>:/workspace/MaximalCAI/

# Directory
scp -r -P 22069 -i ~/.ssh/id_ed25519 local_dir/ root@<redacted-pod-ip>:/workspace/MaximalCAI/

# Using tar for large directories
tar czf - local_dir/ | ssh root@<redacted-pod-ip> -p 22069 -i ~/.ssh/id_ed25519 'tar xzf - -C /workspace/MaximalCAI/'
```

### Download files from pod
```bash
# Single file
scp -P 22069 -i ~/.ssh/id_ed25519 root@<redacted-pod-ip>:/workspace/MaximalCAI/artifacts/results.json ./

# Directory
scp -r -P 22069 -i ~/.ssh/id_ed25519 root@<redacted-pod-ip>:/workspace/MaximalCAI/artifacts/ ./

# Using tar for large directories
ssh root@<redacted-pod-ip> -p 22069 -i ~/.ssh/id_ed25519 'tar czf - /workspace/MaximalCAI/artifacts' | tar xzf -
```
...
# (from local machine)
scp -P 22069 -i ~/.ssh/id_ed25519 root@<redacted-pod-ip>:/workspace/session_backup_*.tar.gz ./
