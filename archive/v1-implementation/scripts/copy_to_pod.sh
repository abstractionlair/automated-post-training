...
DIRECT_SSH="root@<redacted-pod-ip>"
...
cat /Users/user/MaximalCAI/scripts/evaluate_final.py | ssh -p $POD_PORT -i $SSH_KEY -o StrictHostKeyChecking=no $DIRECT_SSH 'cat > /workspace/runs/stage1_20250911_131105/code/scripts/evaluate_final.py' 2>/dev/null

if [ $? -eq 0 ]; then
    echo "✅ Successfully copied evaluate_final.py"
    
    # Copy test instructions too
    cat /Users/user/MaximalCAI/artifacts/held_out_test_instructions_20250911_162708.jsonl | ssh -p $POD_PORT -i $SSH_KEY -o StrictHostKeyChecking=no $DIRECT_SSH 'cat > /workspace/runs/stage1_20250911_131105/code/artifacts/held_out_test_instructions.jsonl'
