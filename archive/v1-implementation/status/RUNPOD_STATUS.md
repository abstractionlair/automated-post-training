...
ssh -p $RUNPOD_PORT -i ~/.ssh/id_ed25519 root@<redacted-pod-ip>
```
...
ssh <redacted-pod-id>@ssh.runpod.io
```
...
```bash
# Upload file
cat /local/path/file | ssh -p $RUNPOD_PORT -i ~/.ssh/id_ed25519 root@<redacted-pod-ip> 'cat > /workspace/path/file'

# Download file
ssh -p $RUNPOD_PORT -i ~/.ssh/id_ed25519 root@<redacted-pod-ip> 'cat /workspace/path/file' > /local/path/file

# Upload directory (tar)
tar czf - /local/dir | ssh -p $RUNPOD_PORT -i ~/.ssh/id_ed25519 root@<redacted-pod-ip> 'tar xzf - -C /workspace/'
```
...
ssh -p $RUNPOD_PORT -i ~/.ssh/id_ed25519 root@<redacted-pod-ip> 'watch -n 1 nvidia-smi'
```
...
ssh -p $RUNPOD_PORT -i ~/.ssh/id_ed25519 root@<redacted-pod-ip> \
  ...
  tar xzf - -C /Users/user/MaximalCAI/
```
...
- Try pinging: `ping <redacted-pod-ip>`
...
# Copy file to pod
cat local_file | ssh -p $RUNPOD_PORT -i ~/.ssh/id_ed25519 root@<redacted-pod-ip> 'cat > /workspace/path'

# Run command on pod
ssh -p $RUNPOD_PORT -i ~/.ssh/id_ed25519 root@<redacted-pod-ip> 'cd /workspace/runs/stage1_20250911_131105/code && command'

# Monitor GPU
ssh -p $RUNPOD_PORT -i ~/.ssh/id_ed25519 root@<redacted-pod-ip> 'nvidia-smi'
```
