# Stage 1 SFT Training - Technical Completion

**Date**: 2025-10-12
**Status**: 🔶 TRAINING COMPLETED — EXPERIMENTAL PREMISE INVALIDATED (see [STAGE1_FINDINGS_AND_PIVOT.md](STAGE1_FINDINGS_AND_PIVOT.md))

> **Reconciliation note**: Training ran successfully and all technical gates passed, but evaluation later showed the base model already followed instructions. The SFT improvement was -0.7% vs base (a null result). See the findings doc for full analysis and project pivot.
...
## Technical Success — Experimental Null Result

Training ran without errors, but the experimental premise was invalid:
...
**Experimental finding** (per `docs/STAGE1_FINDINGS_AND_PIVOT.md`):
- ⚠️ Base model already followed instructions at 100% accuracy
- ⚠️ SFT improvement was -0.7% (marginally worse than base)
- ⚠️ SFT taught response style (brevity), not capability
- ❌ Experimental premise invalidated — pivoted to conversation ability
